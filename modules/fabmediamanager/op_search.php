...
if (!isset($_GET['q'])){
    echo '<div class="alert alert-warning" role="alert">
            ' . $language->get('fabmediamanager', 'searchNoKeywordPassed', null) . '
          </div>';
    //return;
}

$template->navBarAddItem('Mediamanager', $URI->getBaseUri() . 'fabmediamanager/');
$template->navBarAddItem($language->get('fabmediamanager', 'searchTitleSearch', null));
...
<div>
  <div class="form-row">
    ...
    <div class="col-md-1">
        ...
        <button onclick="searchImage();" id="btnSearch" type="submit" class="form-control btn btn-xs btn-primary">' . $language->get('fabmediamanager', 'searchFormSearch', null) . '</button>
    </div>  
  </div>
</div>
...
function searchImage(){
    ...
    $("#searchResult").html("' . $language->get('fabmediamanager', 'searchPleaseWait', null) . '");
        
    $.post( "' . $URI->getBaseUri() . 'fabmediamanager/search_engine/", { q: q, 
                                                                          orderBy: orderBy })
      .done(function( data ) {
